const strtok3 = eval(`import('strtok3')`);
const Token = eval(`import('token-types')`);
import {
  stringToBytes,
...
export async function fileTypeFromBuffer(input) {
  ...
  return fileTypeFromTokenizer((await strtok3).fromBuffer(buffer));
}
...
export async function fileTypeFromTokenizer(tokenizer) {
  try {
    ...
    if (!(error instanceof (await strtok3).EndOfStreamError)) {
      throw error;
    }
  }
}

function _check(buffer, headers, options?: any) {
  options = {
    offset: 0,
    ...options,
  };
...
class FileTypeParser {
  ...
  check(header, options?: any) {
    return _check(this.buffer, header, options);
  }

  checkString(header, options?: any) {
  ...
  async parse(tokenizer) {
      ...
      try {
        while (tokenizer.position + 30 < tokenizer.fileInfo.size) {
          ...
          zipHeader.filename = await tokenizer.readToken(
            new (
              await Token
            ).StringType(zipHeader.filenameLength, 'utf-8'),
          );
          ...
          if (
            ...
            const mimeType = await tokenizer.readToken(
              new (
                await await Token
              ).StringType(zipHeader.compressedSize, 'utf-8'),
            );
    ...
    if (this.check([0x1a, 0x45, 0xdf, 0xa3])) {
      ...
      async function readField() {
        const msb = await tokenizer.peekNumber((await await Token).UINT8);
        let mask = 0x80;
    ...
    if (this.check([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])) {
      ...
      async function readChunkHeader() {
        return {
          length: await tokenizer.readToken((await await Token).INT32_BE),
          type: await tokenizer.readToken(
            new (
              await await Token
            ).StringType(4, 'binary'),
          ),
    ...
    if (
      ...
      async function readHeader() {
        ...
        return {
          id: guid,
          size: Number(
            await tokenizer.readToken((await await Token).UINT64_LE),
          ),
        };
      }
    ...
    if (
      ...
      const type = await tokenizer.readToken(
        new (
          await await Token
        ).StringType(4, 'ascii'),
      );
  ...
  async readTiffTag(bigEndian) {
    const tagId = await this.tokenizer.readToken(
      bigEndian ? (await await Token).UINT16_BE : (await await Token).UINT16_LE,
    );
  ...
  async readTiffHeader(bigEndian) {
    ...
    const ifdOffset = (
      bigEndian ? (await await Token).UINT32_BE : (await Token).UINT32_LE
    ).get(this.buffer, 4);
    ...
    readableStream.once('readable', () => {
      (async () => {
        try {
          ...
          const pass: any = new stream.PassThrough();
